...
export interface Place {
  ...
  latitude: number;
  longitude: number;
  type: 'restaurant' | 'cafe' | 'club' | 'beach' | 'landmark' | 'farm' | 'other';
  rating?: number;
  address?: string;
  description?: string;
  distance?: number;
...
export const placeCategories = {
  restaurant: { name: 'Restoran', icon: '🍽️', color: '#FF6B6B', marker: 'restaurant', osmTag: 'amenity=restaurant' },
  cafe: { name: 'Kafić', icon: '☕', color: '#4ECDC4', marker: 'cafe', osmTag: 'amenity=cafe' },
  club: { name: 'Noćni klub', icon: '🎵', color: '#9B59B6', marker: 'club', osmTag: 'amenity=nightclub' },
  beach: { name: 'Plaža', icon: '🏖️', color: '#3498DB', marker: 'beach', osmTag: 'natural=beach' },
  landmark: { name: 'Znamenitost', icon: '🏛️', color: '#F1C40F', marker: 'landmark', osmTag: 'historic=*' },
  farm: { name: 'OPG', icon: '🌾', color: '#2ECC71', marker: 'farm', osmTag: 'shop=farm' },
  other: { name: 'Ostalo', icon: '📍', color: '#95A5A6', marker: 'other', osmTag: '' },
};
...
const categorizePlaceByTags = (item: any): Place['type'] => {
  ...
  if (type === 'nightclub' || type === 'bar' || class_ === 'nightclub') return 'club';
  if (type === 'beach' || class_ === 'beach') return 'beach';
  if (type === 'historic' || type === 'tourism' || class_ === 'attraction') return 'landmark';
...
export const getPlacesInRadius = async (
  ...
  console.log(`Fetching places for types: ${types.join(', ')}, radius: ${radiusKm}km`);
  
  // 🔥 PRVO POKUŠAJ S BACKEND-OM
  try {
    const backendPlaces = await fetchFromBackend(latitude, longitude, radiusKm, types);
    ...
    try {
      ...
      const places = data.elements
        .map((element: any) => {
          ...
          else if (tags.historic || tags.tourism === 'attraction') placeType = 'landmark';
