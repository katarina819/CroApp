...
// 🔥 OPCIJE ZA RADIJUS (svi rade do 200 km)
...
export default function MapScreen() {
  ...
  // Dohvat mjesta pri promjeni kategorija
  useEffect(() => {
    if (selectedTypes.length > 0 && userLocation) {
      const timeout = setTimeout(() => loadPlaces(), 500);
  ...
  // Dohvat mjesta pri promjeni radijusa
  useEffect(() => {
  ...
  const loadPlaces = async () => {
    ...
    try {
      ...
      // 🔥 Za velike radijuse prikaži upozorenje
      if (radius > 50) {
        setLongLoading(true);
        showNotification(
          `⏳ Učitavanje mjesta u krugu od ${radius} km može potrajati do 60 sekundi...`,
          "info",
        );
      }
      ...
      if (fetchedPlaces.length > 0) {
        ...
        if (mapRef.current) fitMarkersToMap(fetchedPlaces);
      } else {
        showNotification(
  ...
  // 🔥 RENDER KATEGORIJE - GRID 3 PO REDU
